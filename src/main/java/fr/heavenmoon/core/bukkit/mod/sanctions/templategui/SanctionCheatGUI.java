...
public class SanctionCheatGUI extends AbstractGui {

    private String targetName;

    public SanctionCheatGUI(MoonBukkitCore plugin, String target) {
        super(plugin);
        this.targetName = target;
    }

    public void display(Player player) {
        this.inventory = plugin.getServer().createInventory(null, 54, "Infraction (triche)");
        ...
        SanctionList.get(SanctionStatus.TRICHE).forEach(sanction -> {
            double multiplier;
            if (sanction.getSanction().equals(SanctionType.BAN)) {
                ...
                if (sanction == SanctionList.OTHER_LIFE)
                    multiplier = 1;
            } else {
                int history = persistanceManager.getSanctionManager().getAllMutes(customPlayer).size();
                multiplier = Math.exp(history);
            }
            setSlotData(ChatColor.GOLD + sanction.getName(), sanction.getMaterial(), slot.getData(), new String[]{ChatColor.GRAY +
                    "Joueur: " + ChatColor.getByChar(customPlayer.getRankData().getStyleCode()) + customPlayer.getRankData().getRank().getPrefix() + customPlayer.getName(),
                    ChatColor.GRAY + "Durée: " + ChatColor.GREEN + sanction.getPublicTime(multiplier),
                    ChatColor.GRAY + "Sanction: " + ChatColor.RED + sanction.getSanction().getDisplayName(), "", ChatColor.GRAY + "Cliquez pour " +
                    "sanctionner"}, "check");
    ...
    public void onClick(Player player, ItemStack stack, String action, ClickType clickType) {
        if (action.equalsIgnoreCase("close")) {
            ...
            plugin.getGuiManager().openGui(player, new SanctionGUI(plugin, this.targetName));
