...
public class AltsGUI extends AbstractGui {
    ...
    public void display(Player player) {
        ...
        for (int i = 36; i < 45; i++)
            setSlotData(ChatColor.BOLD + "Deco", (new ItemBuilder(Material.STAINED_GLASS_PANE)).setData((byte) 15).toItemStack(), i, null, null);
        setSlotData(ChatColor.LIGHT_PURPLE + "Informations", Material.BOOK, 49, new String[]{ChatColor.GRAY + "Tous les alts disponibles " +
                "concernant:", ChatColor.getByChar(customPlayer.getRankData().getStyleCode()) + customPlayer.getRankData().getRank().getPrefix() + targetName}, null);
    ...
    private String[] makePlayerAltLore(CustomPlayer altPlayer, String alt) {
        ...
        lore.add(ChatColor.GRAY + "Grade: " + ChatColor.getByChar(altPlayer.getRankData().getRank().getStyleCode()) + altPlayer.getRankData().getRank().getName());
        lore.add(ChatColor.GRAY + "Permission: " + ChatColor.getByChar(RankList.getByPermission(altPlayer.getRankData().getPermission()).getStyleCode()) + RankList.getByPermission(altPlayer.getRankData().getPermission()).getName());
