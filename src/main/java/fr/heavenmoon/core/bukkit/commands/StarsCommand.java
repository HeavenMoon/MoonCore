...
public class StarsCommand implements CommandExecutor
{
	...
	public StarsCommand(MoonBukkitCore plugin)
	{
		this.plugin = plugin;
	...
	public boolean onCommand(CommandSender sender, Command command, String s, String[] args)
	{
		if (args.length == 3)
		{
			...
			long stars = Long.parseLong(amount);
			...
			if (args[0].equalsIgnoreCase("add"))
			{
				...
				if (targetPlayer != null)
				{
					...
					new Message(PrefixType.SERVER,
							"Vous avez reçu " + ChatColor.LIGHT_PURPLE + amount + ChatColor.DARK_PURPLE + " Etoiles " + ChatColor.GRAY +
									" !").send(targetPlayer);
				}
				new Message(PrefixType.ADMIN,
						ChatColor.LIGHT_PURPLE + amount + ChatColor.LIGHT_PURPLE + " Etoiles " + ChatColor.RED + "on été ajouté à " +
			...
			else if (args[0].equalsIgnoreCase("remove"))
			{
				...
				if (targetPlayer != null)
				{
					plugin.getScoreboardManager().update(targetPlayer);
					new Message(PrefixType.SERVER,
							"Vous avez perdu " + ChatColor.LIGHT_PURPLE + amount + ChatColor.DARK_PURPLE + " Etoiles " + ChatColor.GRAY +
									" !").send(targetPlayer);
				}
				new Message(PrefixType.ADMIN,
						ChatColor.LIGHT_PURPLE + amount + ChatColor.LIGHT_PURPLE + " Etoiles " + ChatColor.RED + "on été retiré à " +
								ChatColor.DARK_RED + customTarget.getName()).send(sender);
				return false;
