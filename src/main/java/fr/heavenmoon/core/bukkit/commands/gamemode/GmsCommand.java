...
public class GmsCommand implements CommandExecutor
{
	...
	public GmsCommand(MoonBukkitCore plugin)
	{
		this.plugin = plugin;
	...
	public boolean onCommand(CommandSender sender, Command command, String s, String[] args)
	{
		...
		if (args.length == 0)
		{
			customPlayer.setGamemode(Integer.valueOf(0));
			persistanceManager.getPlayerManager().commit(customPlayer);
			player.setGameMode(GameMode.SURVIVAL);
			(new Message(ChatColor.GRAY + "GameMode mis jour: " + ChatColor.GREEN + "SURVIVAL")).send(player);
			return true;
		}
		if (args.length == 1)
		{
			...
			customTarget.setGamemode(Integer.valueOf(0));
			persistanceManager.getPlayerManager().commit(customPlayer);
			target.setGameMode(GameMode.SURVIVAL);
			(new Message(ChatColor.GRAY + "GameMode mis jour: " + ChatColor.GREEN + "SURVIVAL")).send(target);
			(new Message(ChatColor.GRAY + "GameMode mis jour pour " + ChatColor.DARK_GRAY + target.getName() + ChatColor.DARK_GRAY + ": " +
					ChatColor.GREEN + "SURVIVAL")).send(player);
			return true;
		}
		return false;
	}
