...
public class FastReflection {
    public static final String VERSION = Bukkit.getServer().getClass().getPackage().getName().substring("org.bukkit.craftbukkit.".length());
    ...
    private static boolean isClassExistsNMS(String className) {
        try {
            Class.forName("net.minecraft.server." + VERSION + '.' + className);
            return true;
